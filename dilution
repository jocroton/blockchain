...
def new_block(lucky, num):  #creates a new block 
    ...
    info_list[lucky, 1] = 1
    neighbors[lucky] = copy.copy(neighbor_list[lucky]) #start gossiping to all neighbors again
    ...
    for gossiper in gossipers:
         ...
         if len(listeners) == 0 :    #if there are no more neighbors to gossip to
             ...
             if len(last_block[gossiper]) > len(last_block[listener]) : #compare chains (block-ID)
                 ...
                 neighbors[listener] = copy.copy(neighbor_list[listener]) #start gossiping to all neighbors again
                  #need to update the listeners delay times (newly started gossiping)
...
probability_dt = (info_list[:,2]/sum(info_list[:,2]))/100
probability = copy.copy(probability_dt)  #variable that will be changed below
