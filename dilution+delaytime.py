...
###############################################################################
# Loop 1: Dilution rates                  
                      
# Initialize list to record full results                      
fullresults = []

for dilusion_rate in dilusion_rates:
    
    # Initialize list record results for each dilution rate
    results = [] 
    
    ###########################################################################
    # Loop 2: Network delays
    
    for network_delay in network_delays:
        # Initialize lists to record results for each network delay
        res_ratio = []              
        res_orphanedblocks = []
        res_totalblocks = []
        res_onchain = []
        ...
        ######################################################################
        # Loop 3: 10 trials to get the average parameters
        for trial in range(trials): 
            ...
            ###################################################################
            # Record the results for one trial
            
            chain_len = [len(last_block[i]) for i in range(num_nodes)]                          # find the longest chain(s)
            longest_chain_index = [i for i, x in enumerate(chain_len) if x==max(chain_len)]     # record the indices
            longest_chain = copy.copy([last_block[i] for i in longest_chain_index])             # longest chains' block-chains
            newest_block = max([longest_chain[i][-1] for i in range(len(longest_chain))])       # the last block mined in the longest chains
            check_block = list(range(newest_block + 1))                                         # create a comparison chain with no gaps             
            oldest_block = min([longest_chain[i][-1] for i in range(len(longest_chain))])       # check which of the longest chains' last block was mined first
            main_chain = [i[1] for i in enumerate(longest_chain) if i[1][-1] == oldest_block]   # main chain is longest chain with oldest last block
            ...
            avg_consensus_time = consensus_times[-1]/len(consensus_times) # record average time until consensus
            print("num consensus:")
            print(num_consensus)
            print("ratio:")
            ratio = num_orphans/num_total   # record ratio of orphaned blocks
            print(ratio)
            # print(trial)
        ...
        #######################################################################   
        # Record average results for one network delay
        ...
        #  Collect all network delay results
    ...
    # Collect all dilution rate results
